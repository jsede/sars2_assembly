...
while read i; do
bbmap.sh -Xmx"$m"g threads="$t" in=$prj/1_reads/"$i"_trim_1.fastq.gz in2=$prj/1_reads/"$i"_trim_2.fastq.gz outm=$prj/4_remap/"$i".remapped.bam ref=$prj/4_remap/"$i".draft.fa;
done < $prj/IDs.list
